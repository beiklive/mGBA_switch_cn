...
SPDLOG_INLINE int utc_minutes_offset(const std::tm &tm) {
    // Some platforms have _timezone or timezone variable
    // Note: timezone is seconds WEST of UTC, so we need to negate it
    #ifdef _WIN32
        long timezone_seconds = _timezone;
    #elif defined(__APPLE__) || defined(__linux__)
        extern long timezone;
        long timezone_seconds = timezone;
    #else
        // Fallback for platforms without timezone variable
        long timezone_seconds = 0;
    #endif
    
    // Adjust for daylight saving time
    if (tm.tm_isdst > 0) {
        timezone_seconds -= 3600; // Subtract one hour
    }
    
    return static_cast<int>(-timezone_seconds / 60);
}
#endif  // _WIN32
